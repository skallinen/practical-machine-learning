...
The data is downloaded in the working directory. Next we will load it into R. It is important to tell R which values should be interpreted as NAs, hence the `na.strings = c("NA", "#DIV/0!","")` -argument. 
...
Having done that we did some explored the number of NAs in the different columns using the `apply(training, 2, function(x) sum(is.na(x) )` command. It reveals that most of the columns that include any NAs, consist almost entirely of NAs. So we'll remove those columns and therefore get rid of every NA in the entire data set. We have now reduced the number of variables from 160 to 53.
...
Next, we'll split the training data into two sets for cross validation. We'll create a training set (subtraining) that includes 60 % of the entire training data and 40% for cross validation.
...
We'll now do some feature exploration. We actually started with doing principal component analysis, but the prediction results were actually worse than without other results. When we did the preprocessing with default values we got some improvement in accuracy, albeit that those improvements were miniscule. This will be shown in a moment. The improvement was perhaps so slight because accuracy without any preprocessing was already extremely high. We would probably have skipped the preprocessing alltogether, had we known it from the beginning.

**Building the model and doing cross validation**  
The model was built using the random forest algorithm that has been successful when predicting categorical outcomes, as is the case here. We first tried the random forest method using the *train* function in the caret package, but that was tediously slow, and after researching the course forums realized that creating the model using the function directly from the randomForest package is much faster. And indeed it was. It did the calculations in matter of a minute or two. Doint the preprocessing, building the model and testing it on the traing data next.
...
The model predicts with accuracy of 100% on the training data. This signals a risk that the model has been overfitted. We'll find out by running the model on the test data. IShould it perform significantly worse we might have been overfitting.
...
But just to test, we also created a model without preprocessing the data:
...
**Final thoughts**  
If these results are correct, as they seem, it does raise the question how come it predicts so well. Were the trial conditions perhaps to controlled? The accuracy seems almost hard to believe. Also, why are these so much better than results that the reseachers got. They did use another strategy for splitting the data. One cant help but wonder why the researchers did not try this approach as it is rather simple and done almost entirely using defaults.
